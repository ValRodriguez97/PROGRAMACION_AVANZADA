...
export class RegisterComponent {
  ...
  async onNext(): Promise<void> {
    ...
    try {
      ...
      if (this.step < 3) {
        this.step++;
        this.showToast('success', `Paso ${this.step - 1} completado`, 'Continúa con el siguiente paso');
      } else {
        // Registro completo
        const userType = this.formData.isOwner ? 'propietario' : 'usuario';
        this.showToast('success', '¡Cuenta creada exitosamente!', `Bienvenido ${this.formData.fullName} como ${userType}`);
        console.log('Registration complete:', this.formData);
        setTimeout(() => this.router.navigate(['/']), 2000);
      }
    } catch (error) {
      this.showToast('error', 'Error al procesar', 'Por favor intenta nuevamente');
    } finally {
  ...
  onSocialRegister(provider: string): void {
    this.showToast('info', `Registrando con ${provider}...`, 'Esta es una demostración');
  }
